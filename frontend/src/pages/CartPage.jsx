...
/* ===== Breadcrumb (เหมือนหน้า Place: ใช้ isLast) ===== */
...
// ===== helpers: อ่าน/เขียน localStorage อย่างปลอดภัย =====
const readCart = () => {
  try {
...
const writeCart = (arr) => localStorage.setItem(LS_KEY, JSON.stringify(arr));

export default function CartPage() {
  // ⛔ ไม่มี defaultCart อีกต่อไป
  const [cart, setCart] = useState(() => readCart());

  // เผื่อกรณีเปิดหลายแท็บ: sync ตาม storage event
  useEffect(() => {
    const onStorage = (e) => {
      if (e.key === LS_KEY) setCart(readCart());
    };
  ...
  const handleGoCheckout = () => writeCart(cart);
